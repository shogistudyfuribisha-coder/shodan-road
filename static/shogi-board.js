...
function ownerToColor(owner, ShogiAPI) {
  return owner === "S" ? ShogiAPI.Color.Black : ShogiAPI.Color.White;
}
...
function setupBoardPieces(board, question, ShogiAPI) {
  question.board.forEach((row, rowIndex) => {
    row.forEach((piece, colIndex) => {
      ...
      const { x, y } = questionBoardToShogi(rowIndex, colIndex);

      const color = ownerToColor(owner, ShogiAPI);

      const csa = color === ShogiAPI.Color.Black ? `+${kind}` : `-${kind}`;

      board.set(x, y, new ShogiAPI.Piece(csa));
    });
  });
}

function setupHands(board, question, ShogiAPI) {
  setupHand(board, question.hand.先手, ShogiAPI.Color.Black, ShogiAPI);

  setupHand(board, question.hand.後手, ShogiAPI.Color.White, ShogiAPI);
}
...
function setupHand(board, pieces, color, ShogiAPI) {
  pieces.forEach((pieceName) => {
    ...
    board.pushToHand(new ShogiAPI.Piece(csa));
  });
}

function setupTurn(board, question, ShogiAPI) {
  const move = question.type === "3ply" ? question.moves[0] : question.move;

  if (move.from_hand) {
    const owner = move.piece.charAt(0);

    board.setTurn(ownerToColor(owner, ShogiAPI));
  ...
  const { row, col } = shogiToQuestionBoard(file, rank);

  const piece = question.board[row][col];

  if (!piece) {
    throw new Error(`移動元に駒がありません: ${file}${rank}`);
  }
...
function dropPiece(board, move, ShogiAPI) {
  ...
  board.drop(file, rank, kind, color);
}
...
function movePiece(board, move) {
  ...
  board.move(fromFile, fromRank, toFile, toRank, move.promote || false);
}
