...
describe("shogi.js", () => {
  ...
  test("20問すべての盤面をshogi.jsで再現できる", () => {
    questions.forEach((question) => {
      const board = shogiBoard.createShogiBoard(question, {
        Shogi,
        Color,
        Piece,
      });
      ...
      question.board.forEach((row, rowIndex) => {
        row.forEach((piece, colIndex) => {
          const { x, y } = shogiBoard.questionBoardToShogi(rowIndex, colIndex);
          ...
          expect(actual.kind).toBe(shogiBoard.PIECE_KIND_MAP[pieceName]);

          expect(actual.color).toBe(shogiBoard.ownerToColor(owner, { Color }));
        });
      });
    });
  });
  ...
  test("20問すべての正解手をshogi.jsで実行できる", () => {
    questions.forEach((question) => {
      ...
      expect(() => {
        shogiBoard.executeCorrectMove(board, question.move, {
          Shogi,
          Color,
          Piece,
        });
  ...
  test("20問すべての正解手を実行した後の盤面が正しい", () => {
    questions.forEach((question) => {
      ...
      if (move.from_hand) {
        ...
        expectedKind = shogiBoard.PIECE_KIND_MAP[pieceName];

        expectedColor = shogiBoard.ownerToColor(owner, { Color });
      } else {
        const [fromFile, fromRank] = move.from;

        const movingPiece = board.get(fromFile, fromRank);
      ...
      if (!move.from_hand) {
        ...
        expect(board.get(fromFile, fromRank)).toBeNull();
      }
    });
  });
  ...
  test("持ち駒から駒を打てる", () => {
    ...
    expect(board.get(5, 2)).toBeNull();

    board.drop(5, 2, "KI", Color.Black);
  ...
  test("持ち駒から駒を打つと持ち駒が1枚減る", () => {
    ...
    expect(board.hands[Color.Black]).toHaveLength(2);

    board.drop(5, 2, "KI", Color.Black);

    expect(board.hands[Color.Black]).toHaveLength(1);

    expect(board.get(5, 2).kind).toBe("KI");
  });
  ...
  test("持ち駒を打った後の盤面と持ち駒が正しい", () => {
    ...
    expect(board.hands[Color.Black]).toHaveLength(0);
  });
  ...
  test("盤上の正しい着手を正解と判定できる", () => {
    ...
    expect(shogiQuestion.isCorrectMove(userMove, correctMove)).toBe(true);
  });
